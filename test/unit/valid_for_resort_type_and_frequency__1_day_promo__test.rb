...
class ValidForResortTypeAndFrequency_1DayPromo_Test < ActiveSupport::TestCase
  def test_1
    ...
    time = time + 1000
    ticket_1_day_promo__B = Ticket.find_by_code('ticket_1_day_promo__B')
    scan = non_promo_resort.scan_lift_ticket_at_timestamp(ticket_1_day_promo__B, time)
    self.assert(!scan.valid_for_resort_type_and_frequency?)

    time = time + 1000
    ticket_1_day_promo__A = Ticket.find_by_code('ticket_1_day_promo__A')
    scan = promo_resort.scan_lift_ticket_at_timestamp(ticket_1_day_promo__A, time)
    self.assert(!scan.valid_for_resort_type_and_frequency?)
